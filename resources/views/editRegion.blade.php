...
@section('content')
<legend class="fs-2 ml-4 mt-3">Edit Region</legend>
<ul class=" user nav mr-5">
    <li class="nav-item">
      <a class="nav-link" href="{{ route('user.addregion') }}">Add Regions</a>
    </li>
  </ul>
    ...
    <div class="col-3 col-lg-6">
       <div class="card px-0 pb-0 shadow p-3">
           <div class="justify-content-center d-flex">
            ...
            <form action="{{ route('user.updateRegion') }}" method="POST">
                @csrf
                <input type="hidden" name="id" value="{{ $region['id'] }}">
                <div class="form-group ml-3 mr-3">
                    <label for="county" class="form-label">County</label>
                    <input type="text" name="county" class="form-control" value="{{ $region['county'] }}">
                    <span class="text-danger">@error('county'){{ $message }}@enderror</span>
                </div>
                <div class="form-group ml-3 mr-3">
                    <label for="area" class="form-label">Area</label>
                    <input type="text" name="area" class="form-control" value="{{ $region['area'] }}">
